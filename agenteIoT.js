...
// ========================================================
// FUNÇÃO AUXILIAR: Formatar segundos em MM:SS
// ========================================================
...
function atualizarPainel() {
    ...
    listaDeVoos.forEach(voo => {
        ...
        novoCard.innerHTML = `
            ...
            <p id="tempo-${voo.codigo}" style="font-size: 20px; font-weight: bold; color: #2ecc71; margin: 10px 0;">⏱️ ${formatarTempo(voo.tempoRestante)}</p>
        `
        ...
        // Iniciar o temporizador para este voo
        iniciarTemporizador(voo.codigo)
    })
}

// ========================================================
// FUNÇÃO: INICIAR TEMPORIZADOR PARA CADA VOO
// ========================================================
function iniciarTemporizador(codigoVoo) {
    setInterval(() => {
        let voo = listaDeVoos.find(v => v.codigo === codigoVoo)
        if (voo && voo.tempoRestante > 0) {
            voo.tempoRestante--
            let elementoTempo = document.getElementById(`tempo-${codigoVoo}`)
            if (elementoTempo) {
                elementoTempo.innerText = `⏱️ ${formatarTempo(voo.tempoRestante)}`
                // Mudar cor conforme o tempo passa
                if (voo.tempoRestante <= 60) {
                    elementoTempo.style.color = "#e74c3c"
                } else if (voo.tempoRestante <= 180) {
                    elementoTempo.style.color = "#f39c12"
                }
            }
        }
    }, 1000)
